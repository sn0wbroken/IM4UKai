...
#pragma region CreateMorphMeshStreams
		...
		{
			MorphTarget->MorphLODModels.AddDefaulted(LODIndex - MorphTarget->MorphLODModels.Num() + 1);
		}
		...
		MorphModel.NumBaseMeshVerts = BaseMeshRawData.Vertices.Num();
		...
		WasProcessed.Empty(BaseMeshRawData.Vertices.Num());
		WasProcessed.AddZeroed(BaseMeshRawData.Vertices.Num());
		...
		for (int32 Idx = 0; Idx < TargetMeshRawData.WedgePointIndices.Num(); Idx++)
		{
			if (BaseMeshRawData.WedgePointIndices.IsValidIndex(Idx) && BaseMeshRawData.WedgePointIndices[Idx] == TargetMeshRawData.WedgePointIndices[Idx])
			{
				WedgePointToVertexIndexMap.Add(TargetMeshRawData.WedgePointIndices[Idx], Idx);
			}
		}

		// iterate over all the base mesh indices
		for (int32 Idx = 0; Idx < BaseMeshRawData.Indices.Num(); Idx++)
		{
			uint32 BaseVertIdx = BaseMeshRawData.Indices[Idx];
			...
			if (!WasProcessed[BaseVertIdx])
			{
				...
				const FMorphMeshVertexRaw& VBase = BaseMeshRawData.Vertices[BaseVertIdx];

				// clothing can add extra verts, and we won't have source point, so we ignore those
				if (BaseMeshRawData.WedgePointIndices.IsValidIndex(BaseVertIdx))
				{
					// get the base mesh's original wedge point index
					uint32 BasePointIdx = BaseMeshRawData.WedgePointIndices[BaseVertIdx];
					...
					if (TargetVertIdx != NULL)
					{
						...
						const FMorphMeshVertexRaw& VTarget = TargetMeshRawData.Vertices[*TargetVertIdx];
